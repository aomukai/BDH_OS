...
The final workstation archive contains 12,329 files (approximately 1.7 GB), including both legacy workstation ledgers and seven obsolete Ninereeds unit definitions. The final trainbox archive contains 11,782 files (approximately 1.8 GB), including host-local visual assets, migration/evidence staging, the legacy trainbox ledger, and five obsolete trainbox unit definitions. Both remote host-state transfers passed checksum dry runs with zero unpreserved files.

The trainbox heartbeat and legacy worker units were stopped, disabled, and removed after preservation. Its Ninereeds share directory now contains only the clean role candidate. Mission Hub's database and evidence store remain in their authoritative location. Python bytecode and pytest caches were deleted as reproducible non-evidence output; no other workstation material was deleted.
