...
# Published research visualizations are self-contained HTML files generated by
# the training pipeline. They intentionally embed inline CSS/JS and, for 3D
# views, may use eval/blob-backed browser machinery from bundled visualization
# libraries. Keep the main Lab application on the strict default CSP above, but
# allow these authenticated artifact documents to execute their own contents.
HTML_ARTIFACT_CSP = (
    "default-src 'self' data: blob:; base-uri 'none'; object-src 'none'; "
    "frame-ancestors 'self'; frame-src 'self'; img-src 'self' data: blob:; "
    "style-src 'self' 'unsafe-inline'; "
    "script-src 'self' 'unsafe-inline' 'unsafe-eval' blob:; "
    "worker-src 'self' blob:; connect-src 'self' ws: wss:; form-action 'self'"
)
...
class LabRuntime:
    ...
    def _publish_attention_updates(self) -> None:
        ...
        development = dashboard.get("development_state") or {}
        recommendation = (
            development.get("recommended_next_action")
...
class LabHandler(BaseHTTPRequestHandler):
    ...
    def end_headers(self) -> None:
        ...
        self.send_header(
            ...
            getattr(self, "_content_security_policy", DEFAULT_CSP),
        )
        if hasattr(self, "_content_security_policy"):
            delattr(self, "_content_security_policy")
        super().end_headers()
    ...
    def _serve_repo_file(self, relative_path: str, forced_type: str | None = None, head_only: bool = False) -> None:
        ...
        if "html" in content_type:
            self._content_security_policy = HTML_ARTIFACT_CSP
