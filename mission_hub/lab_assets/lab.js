...
const JOB_CATEGORY_ORDER = ["Campaign planning", "Training material", "Visual learning", "Model development", "System & safety"];
...
function renderDashboard() {
  ...
  $("#pipelineControlLabel").textContent = pipeline.effective_state === "pausing" ? "Scheduler disarming" : pipeline.effective_state === "starting" ? "Scheduler arming" : pipeline.desired_state !== "running" ? "Scheduler paused" : live ? "Scheduler armed · active" : next ? "Scheduler armed · work queued" : "Scheduler armed · idle";
...
function renderSettings() {
  ...
  const modelCards = data.models.map((model) => { const textModel = (model.modality || "text") === "text" || model.modality === "vision_language"; const limits = textModel ? `<label>Maximum context ${helpTip("The largest combined prompt and conversation the model can accept, measured in tokens.")}<input type="number" min="1" data-model-field="context_tokens" value="${model.context_tokens}"></label><label>Maximum answer ${helpTip("The most tokens Mission Hub may allow the model to generate in one response.")}<input type="number" min="1" data-model-field="output_tokens" value="${model.output_tokens}"></label>` : `<label>Immutable revision<input data-model-field="revision" value="${escapeHTML(model.revision)}"></label><label>Model role<input value="${escapeHTML(friendlyIdentifier(model.modality))}" disabled></label>`; return `<article class="setting-card" data-model-card="${escapeHTML(model.id)}"><div class="setting-head"><div><p class="technical-id">${escapeHTML(model.id)} · ${escapeHTML(PROVIDER_NAMES[model.provider] || friendlyIdentifier(model.provider))}</p><h2>${escapeHTML(friendlyIdentifier(model.id))}</h2><p class="muted">${escapeHTML(friendlyIdentifier(model.modality || "text"))} model${model.revision ? ` · pinned ${model.revision.slice(0, 12)}…` : ""}</p></div><label class="toggle"><input type="checkbox" data-model-field="enabled" ${model.enabled ? "checked" : ""}> Available for use</label></div><div class="setting-grid"><label>Provider's exact model name ${helpTip("Copy this identifier exactly from the provider. Similar-looking names may refer to different models.")}<input data-model-field="exact_name" value="${escapeHTML(model.exact_name)}"></label>${limits}</div></article>`; }).join("");
  $("#settingsProviders").innerHTML = `<div class="settings-actionbar"><div><h2>Model services</h2><p>Connections tell Mission Hub where models live. Secrets stay outside the Lab.</p></div><button id="addProviderButton" class="quiet-button">Add a service</button></div><div class="settings-card-stack">${providerCards}</div><div class="settings-actionbar sub"><div><h2>Models not listed</h2><p>These requested limits are used when you select a newly discovered or manually named model. A known provider ceiling always wins if it is lower.</p></div></div><article class="setting-card" data-model-defaults-card><div class="setting-grid two"><label>Requested context tokens ${helpTip("Used when adding a model. If the provider reports a smaller context window, Mission Hub saves that smaller value instead.")}<input type="number" min="1" data-model-default-field="unlisted_context_tokens" value="${data.model_defaults.unlisted_context_tokens}"></label><label>Requested output tokens ${helpTip("Used when adding a model. If the provider reports a smaller output ceiling, Mission Hub saves that smaller value instead.")}<input type="number" min="1" data-model-default-field="unlisted_output_tokens" value="${data.model_defaults.unlisted_output_tokens}"></label></div></article><div class="settings-actionbar sub"><div><h2>Models</h2><p>Each selectable model belongs to one service above.</p></div><button id="addModelButton" class="quiet-button">Add a model</button></div><div class="settings-card-stack">${modelCards || '<p class="muted">No models configured.</p>'}</div>`;
  $("#settingsPrompts").innerHTML = data.prompts.filter((prompt) => prompt.id !== "none").map((prompt) => { const presentation = JOB_PRESENTATION[prompt.job_type]; const deterministic = data.jobs.find((job) => job.id === prompt.job_type)?.provider_route === "deterministic"; const description = deterministic ? "This prefilled task contract documents the bounded deterministic check. No language model receives it." : "These are the instructions sent when this job asks a language model for help."; return `<article class="setting-card" data-prompt-card="${escapeHTML(prompt.id)}"><div class="setting-head"><div><p class="technical-id">${escapeHTML(prompt.id)} · version ${prompt.version}</p><h2>${escapeHTML(presentation?.title || friendlyIdentifier(prompt.job_type))}</h2><p class="muted">${escapeHTML(description)}</p></div><label class="toggle"><input type="checkbox" data-prompt-field="enabled" ${prompt.enabled ? "checked" : ""}> Available for use</label></div><div class="prompt-fields"><label>Role and rules ${helpTip("Stable instructions that define the task's role, boundaries, and required behavior.")}<textarea data-prompt-field="system">${escapeHTML(prompt.system)}</textarea></label><label>Task template ${helpTip("The job-specific request. Placeholders are filled from the approved job input when it runs.")}<textarea data-prompt-field="template">${escapeHTML(prompt.template)}</textarea></label></div></article>`; }).join("");
  bindSettingsInputs();
}
