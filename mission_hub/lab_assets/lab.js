...
function ensureCodexModel(modelId) {
  ...
  if (!catalog) return;
  state.settingsWorking.models.push({ id: modelId, provider: "codex-headless", exact_name: catalog.id, enabled: false, local: false, context_tokens: catalog.context_tokens || 128000, output_tokens: 8192, structured_output: true, runtime: "codex exec", weights: "", device: "remote", modality: "text", revision: "" });
}
...
function renderSettings() {
  ...
  const options = (selected, route) => `<option value="">No model</option>${selectableModels.filter((model) => !route?.model_modalities?.length || route.model_modalities.includes(model.modality || "text")).map((model) => `<option value="${escapeHTML(model.id)}" ${model.id === selected ? "selected" : ""}>${escapeHTML(model.catalog_name || friendlyIdentifier(model.id))} · ${escapeHTML(PROVIDER_NAMES[model.provider] || friendlyIdentifier(model.provider))}</option>`).join("")}`;
  ...
  const budgetCard = `<div class="settings-actionbar sub"><div><h2>External-model budget</h2><p>External calls stay locked unless both the switch and positive weekly/monthly ceilings are saved and commissioned.</p></div></div><article class="setting-card" data-budget-card><div class="setting-grid"><label>Allow external calls<select data-budget-field="external_calls_enabled"><option value="false" ${!data.budget.external_calls_enabled ? "selected" : ""}>No</option><option value="true" ${data.budget.external_calls_enabled ? "selected" : ""}>Yes</option></select></label><label>Monthly ceiling (USD)<input type="number" min="0" step="0.01" data-budget-field="monthly_limit" value="${data.budget.monthly_limit}"></label><label>Weekly ceiling (USD)<input type="number" min="0" step="0.01" data-budget-field="weekly_limit" value="${data.budget.weekly_limit}"></label><label>Require per-run approval above (USD)<input type="number" min="0" step="0.01" data-budget-field="per_run_approval_above" value="${data.budget.per_run_approval_above}"></label><label>Emergency reserve (USD)<input type="number" min="0" step="0.01" data-budget-field="emergency_reserve" value="${data.budget.emergency_reserve}"></label><label>Warning fraction<input type="number" min="0" max="1" step="0.01" data-budget-field="warning_fraction" value="${data.budget.warning_fraction}"></label><label>Restriction fraction<input type="number" min="0" max="1" step="0.01" data-budget-field="restriction_fraction" value="${data.budget.restriction_fraction}"></label><label>Hard-stop fraction<input type="number" min="0" max="1" step="0.01" data-budget-field="hard_stop_fraction" value="${data.budget.hard_stop_fraction}"></label></div></article>`;
