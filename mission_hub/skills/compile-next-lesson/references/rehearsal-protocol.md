...
There is no monolithic presentation pass. For every controlled gate, Luna emits exactly the
presentation items bound to that gate immediately before its first scored prompt. The next gate's
model is withheld until the current gate terminates. `PRESENT_AGAIN` may later replay only that
same gate's mapped local model before its frozen cold retest.
...
Scored work is brokered one exercise at a time. A teacher artifact may contain a multi-turn model
and its unscored interface check, but it may contain at most one scored exercise. The broker admits
that teacher/learner pair and resolves any licensed intervention before requesting or admitting the
next scored item. This keeps `PRESENT_AGAIN` adjacent to the failed item instead of attempting a
late rewind after later items have advanced the execution cursor. Every teacher artifact is linted
in a fail-stop operation: append is a separate operation performed only after a successful lint
exit. A shell or workflow must never continue from failed preflight into append.
